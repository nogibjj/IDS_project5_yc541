...
def insertDB(cursor, job_id, company_name, job_name, posted_date, job_type, position_count):
    cursor.execute("INSERT INTO jobs (job_id, company_name, job_name, posted_date, job_type, position_count) VALUES (?, ?, ?, ?, ?, ?)",
...
def updateDB(cursor, job_id, new_posted_date):
    cursor.execute("UPDATE jobs SET posted_date = ? WHERE job_id = ?", (new_posted_date, job_id))
...
def main():
    ...
    # C
    createDB(cursor)
    insertDB(cursor, 55176, "Amazon", "Backend Developer", "2023-09-28", "experienced", 500)
    insertDB(cursor, 55176, "Amazon", "Associate Software Develope", "2023-09-27", "new grads", 400)
    insertDB(cursor, 87356, "LinkedIn", "Fullstack Developer", "2023-09-27", "new grads", 300)
    insertDB(cursor, 99345, "Google", "Technolog Analyst", "2023-09-26", "intern", 200)
    insertDB(cursor, 70001, "Apple", "Software Engineer", "2023-09-25", "new grads", 100)
    insertDB(cursor, 80002, "Oracle", "Associate Software Developer", "2023-09-24", "experienced", 250)
    insertDB(cursor, 90102, "Veeva System", "Associate Software Developer", "2023-09-24", "new grads", 350)
